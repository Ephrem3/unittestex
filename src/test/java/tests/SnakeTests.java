...
public class SnakeTests {
    ...
    {
        actualSnake = new Snake(AnimalType.WILD, Skin.SCALES, Gender.FEMALE, Breed.BURMESE);
    }


    @Test
    @Order(1)
    @DisplayName("Animal Test Type Tests Wild")
    public void animalTypeTests()
    {
        createAnimals();
        assertEquals(AnimalType.WILD, actualSnake.getAnimalType(), "Animal Type Expected[" + AnimalType.WILD
                + "] Actual[" + actualSnake.getAnimalType() + "]");
    }

    @Test
    @Order(1)
    @DisplayName("Snake Speak hiss Test 1")
